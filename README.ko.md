...
라이트 브레인은 손으로 설계한 11→6→2 신경망으로 먹이와 벽, 배고픔을
감지해 다음 걸음을 정합니다. 리얼 브레인은 끝까지 갑니다. 실제 302뉴런
...
- **꼭 필요한 만큼만** — 스탯 2개(포만감, 기분)와 할 일 3가지:
  ...
  - 🎨 **스킨** — 4가지(기본, 베리, 허니, 고스트). 길이·체형·무늬·얼굴이
    각각 다르고, 고른 건 껐다 켜도 유지됩니다
- **시스템 연동** — CPU가 바쁘면 빨라지고, 배터리가 줄면 작아지고, 충전기
  빼면 꼬리가 짧아집니다
- **가벼움** — M4 Pro, Release 기준 CPU 3~5%, 메모리 100MB(리얼 엔진.
  라이트는 더 가볍습니다)
...
🐛 메뉴 맨 위에는 벌레의 현재 스탯(포만감, 기분)이 보입니다.
...
Tests/OhMyWormTests/    유닛 테스트 41개 (엔진, 모델, 컨트롤러, vitals)
