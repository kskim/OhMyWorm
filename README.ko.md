...
돌아다닙니다. 움직임은 신경망이 정합니다. 손으로 짠 작은 신경망부터
실제 302뉴런 엔진까지, 세 가지 브레인 중에 고를 수 있습니다. 밥 주고,
만지고, 스킨을 바꿔 주세요. 창도, 설정도, 계정도 없습니다.

## 배경

예쁜꼬마선충(*C. elegans*) 커넥톰에서 영감을 받았습니다.

라이트 브레인은 손으로 설계한 13→6→2 신경망으로 먹이와 벽, 커서,
배고픔을 감지해 다음 걸음을 정합니다. 리얼 브레인은 끝까지 갑니다.
실제 302뉴런
커넥톰에 95개 근육층, 실제 운동뉴런으로 실행하는 klinotaxis 조향까지
얹었습니다.
...
- **신경망 이동** — 신경망이 감각 입력으로 방향을 정합니다. 가장자리를
  선호하고, 마우스 커서가 가까우면 피합니다. 입력이 같으면 경로는 항상
  같습니다
- **엔진 2종** — 라이트(미니 신경망)와 리얼(302뉴런+근육+먹이추적).
  메뉴에서 변경
- **꼭 필요한 만큼만** — 스탯 2개(포만감, 기분)와 할 일 3가지:
  - 🍎 **밥주기** — 근처에 랜덤 과일이 떨어지면 벌레가 찾아가서 먹습니다.
    포만감이 0이 되면 알아서 하나 떨어집니다
  - ❤️ **만지기** — 벌레를 클릭하거나 드래그로 들어 옮기세요
  - 🎨 **스킨** — 3가지(예쁜꼬마선충, 드레곤, 방울뱀). 길이·체형·무늬·얼굴이
    각각 다르고, 고른 건 껐다 켜도 유지됩니다
- **시스템 연동** — CPU가 바쁘면 빨라지고, 배터리가 줄면 작아지고, 충전기
  빼면 꼬리가 짧아집니다
- **가벼움** — M4 Pro, Release 기준 CPU 5~8%, 메모리 100MB(리얼 엔진.
  라이트는 더 가볍습니다)

## 요구 사항

- macOS 14 이상 (Apple Silicon·Intel Universal)
- 직접 빌드하려면 Xcode 16 이상

## 설치

[릴리즈 페이지](https://github.com/kskim/OhMyWorm/releases)에서 `.dmg`를 받아
여세요. `OhMyWorm`을 `Applications`에 끌어다 놓으면 끝입니다.

서명되지 않은 앱이라 처음에는 우클릭 → 열기로 실행하세요. (또는
`xattr -dr com.apple.quarantine /Applications/OhMyWorm.app` 실행)
...
🐛 메뉴 맨 위에는 벌레의 현재 스탯(포만감, 기분)이 보입니다. 메뉴는
시스템 언어를 따라갑니다. 한국어 시스템에서는 한국어, 그 외에는 영어로
나옵니다.
...
App/
  ...
  Pet/
    ...
    MiniBrain.swift        라이트: 손설계 13→6→2 신경망
    Real/                  리얼: 302뉴런 + 근육 + 조향 제어
    LocomotionEngine.swift 엔진 프로토콜 + 선택
    DesktopPanel.swift   벌레를 따라다니는 클릭-스루 오버레이
    WormView.swift       Canvas 렌더링
    PetMenu.swift        하나뿐인 메뉴
    L10n.swift           시스템 언어에 따른 한/영 문자열
  Resources/connectome.json 동봉된 배선 데이터 (docs 참고)
  Assets.xcassets       앱 아이콘 세트
Tests/OhMyWormTests/    유닛 테스트 50개 (엔진, 모델, 컨트롤러, vitals)
Tools/GenerateIcon.swift 아이콘 재생성 스크립트
...
- 지금은 메인 디스플레이만 지원합니다.
- 메뉴는 한/영 2개 국어를 지원하고 시스템 언어를 따라갑니다.
